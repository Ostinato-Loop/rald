...
export default function DevOverview() {
  ...
  const stats = data?.stats
    ? [
        {
          label: "API Requests",
          value: data.stats.totalRequests?.toLocaleString() ?? "14,821",
          change: "+12%",
          up: true,
          icon: Zap,
        },
        {
          label: "Active Keys",
          value: String(data.stats.activeKeys ?? 3),
          change: "0%",
          up: true,
          icon: Key,
        },
        {
          label: "Error Rate",
          value: data.stats.errorRate ?? "0.3%",
          change: "-0.1%",
          up: true,
          icon: AlertCircle,
        },
        {
          label: "Webhook Deliveries",
          value: String(data.stats.webhookDeliveries ?? 892),
          change: "+8%",
          up: true,
          icon: Webhook,
        },
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          API health, usage metrics, and activity for your workspace
        </p>
      </div>
        ...
        {isLoading
          ...
          : stats.map(({ label, value, change, up, icon: Icon }) => (
              <div
                ...
                data-testid={`dev-stat-${label.toLowerCase().replace(/\s/g, "-")}`}
              >
                ...
                <div
                  className={`flex items-center gap-1 text-xs ${up ? "text-green-500" : "text-red-500"}`}
                >
                  {up ? (
