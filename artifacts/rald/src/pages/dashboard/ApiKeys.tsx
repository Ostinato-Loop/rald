...
export default function ApiKeys() {
  ...
  const revokeMutation = useMutation({
    mutationFn: (id: string) => apiCall(`/api-keys/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["api-keys"] });
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Manage access credentials for your applications</p>
        </div>
        <button
      ...
      {showCreate && (
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          ...
          <div>
            <label className="text-xs text-muted-foreground font-medium uppercase tracking-wide block mb-1.5">Key name</label>
            <input
              value={newKeyName}
          ...
          <div>
            <label className="text-xs text-muted-foreground font-medium uppercase tracking-wide block mb-2">Scopes</label>
          ...
          <div key={k.id} className="bg-card border border-border rounded-xl p-5 space-y-3" data-testid={`api-key-${k.id}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${k.status === "active" ? "bg-green-500/10 text-green-500 border border-green-500/20" : "bg-red-500/10 text-red-500 border border-red-500/20"}`}>
                  {k.status}
                </span>
              </div>
              <button
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
              <div className="flex flex-wrap gap-1">
                {k.scopes.map(s => (
                  <span key={s} className="font-mono bg-muted px-1.5 py-0.5 rounded text-[10px]">{s}</span>
                ))}
              </div>
