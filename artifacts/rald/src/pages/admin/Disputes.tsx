...
const MOCK_DISPUTES = [
  ...
  {
    id: "d3",
    ref: "ESC-2026050300007",
    buyer: "+2348022233344",
    seller: "+2348011122233",
    amount: 40000,
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Manage escrow disputes and mediation
        </p>
      </div>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              ...
              <div>
                ...
                <p
                  className={`mt-0.5 capitalize ${d.evidence === "submitted" ? "text-green-500" : "text-yellow-500"}`}
                >
                  {d.evidence}
                </p>
              </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2 bg-primary text-primary-foreground text-xs font-semibold rounded-lg hover:opacity-90"
                data-testid={`resolve-buyer-${d.id}`}
              >
                Release to Buyer
              </button>
              <button
                onClick={() => resolve(d.id, "seller")}
                className="flex-1 py-2 border border-border text-foreground text-xs font-medium rounded-lg hover:bg-muted"
                data-testid={`resolve-seller-${d.id}`}
              >
                Release to Seller
              </button>
            </div>
