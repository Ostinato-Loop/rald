    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Manage all RALD accounts across the platform
        </p>
      </div>
        ...
        <div className="flex bg-muted rounded-lg p-0.5">
          {["all", "active", "suspended", "pending"].map((s) => (
            <button
              key={s}
              onClick={() => setStatus(s)}
              className={`px-2.5 py-1 text-xs font-medium rounded-md capitalize transition-colors ${status === s ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"}`}
            >
              {s}
            </button>
          ))}
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((u) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full capitalize ${statusColors[u.status]}`}
                    >
                      {u.status}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-semibold ${riskColor(u.riskScore)}`}
                    >
                      {u.riskScore}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {u.status !== "suspended" ? (
                        ...
                        <button
                          className="p-1 text-muted-foreground hover:text-green-500 rounded"
                          onClick={(e) => {
                            e.stopPropagation();
                          }}
        ...
        >
          ...
          >
            ...
            <div className="flex gap-2">
              <button className="flex-1 py-2 border border-border text-foreground text-sm rounded-lg hover:bg-muted">
                Revoke Sessions
              </button>
              <button className="flex-1 py-2 bg-destructive text-destructive-foreground text-sm font-medium rounded-lg hover:opacity-90">
                Suspend
              </button>
            </div>
          </div>
