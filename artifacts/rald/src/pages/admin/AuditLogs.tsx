...
export default function AuditLogs() {
  ...
  const filtered = MOCK_AUDIT.filter((l) => {
    ...
    if (search && !l.action.includes(search) && !l.actor.includes(search))
      return false;
    return true;
  });
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Immutable record of all actions taken on the platform
        </p>
      </div>
        ...
        <div className="flex bg-muted rounded-lg p-0.5">
          {SEVERITIES.map((s) => (
            <button
              ...
              className={`px-2.5 py-1 text-xs font-medium rounded-md capitalize transition-colors ${severity === s ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"}`}
            >
              {s}
            </button>
          ))}
          ...
          <table className="w-full text-xs font-mono">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                {["Severity", "Action", "Actor", "Resource", "IP", "Time"].map(
                  (h) => (
                    <th
                      key={h}
                      className="text-left px-4 py-3 text-muted-foreground font-medium not-italic"
                    >
                      {h}
                    </th>
                  ),
